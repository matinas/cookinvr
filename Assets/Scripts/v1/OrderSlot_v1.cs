...
public class OrderSlot_v1 : MonoBehaviour {
	...
	void Awake()
	{
		GetComponentInParent<OMController_v1>().onOMDispatchSuccess += RemoveOrder;
	}
	...
	void OnTriggerEnter(Collider col)
	{
		if (!isBeingUsed)
		{
			o = col.gameObject.GetComponent<Order_v1>();
			if (o != null)
			{
				isBeingUsed = true;
	...
	void OnTriggerExit(Collider col)
	{
		if (isBeingUsed)
		{
			Order_v1 order = col.gameObject.GetComponent<Order_v1>();
			if (order == o)
			{
				isBeingUsed = false;
				o.onOrderDropped -= HandleOrderDrop;
